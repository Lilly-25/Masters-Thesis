...
\begin{enumerate}
    ...
    % add last bit to hyperparameter tuning
    % So the 2 hidden sizes which we can experiment with are typically 64 and 128. 
...
\makebox[\textwidth][c]{%
    \begin{minipage}{0.4\textwidth}
        ...
        \textit{where}
        \begin{itemize}[label={}]
            \item $\lambda_{\text{y1}}$: Y1 Loss Regularization Weight
            \item $\lambda_{\text{y2}}$: Y2 Loss Regularization Weight
            \item w: Y1 Loss Weightage
            \item 1 - w: Y2 Loss Weightage
    ...
    \begin{minipage}{0.4\textwidth}
        ...
        \textit{where
                \begin{itemize}
                    \item n : \ac{EM} samples
                    \item h : dimensionality of 1D vector
                    \item y1 rmse: \ac{RMSE} for each test sample
                \end{itemize}
                }
    \end{minipage}
}
...
\makebox[\textwidth][c]{%
    \begin{minipage}{0.4\textwidth}
        ...
        \textit{where
                \begin{itemize}
                    \item n : \ac{EM} samples
                    \item w : 1st dimension of 2D vector
                    \item h : 2nd dimension of 2D vector
                    \item y2 rmse : \ac{RMSE} for each test sample
                \end{itemize}
                }
    \end{minipage}
}
...
\makebox[\textwidth][c]{%
    \begin{minipage}{0.4\textwidth}
        ...
        \textit{where
                \begin{itemize}
                    \item n : \ac{EM} samples
                    \item w : 1st dimension of 1D vector
...
% \subsection{Mgrenz \ac{KPI} Results with Baseline}\label{sec:2D Torque Curve Results with Baseline}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=1\textwidth]{./ReportImages/KPI2D_predictions.png} 
%     \caption{Baseline Results for Mgrenz \ac{KPI}} 
%     \label{fig:MLP Training Results for 2D KPI(Mgrenz)}
% \end{figure}
