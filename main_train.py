...
def main(max_torque=None, min_torque=None):
    ##Data Preprocessing
    ...
    for fold, (train_index, val_index) in enumerate(splits_indices, 1):
        ...
        wandb.init(project="EM", config={
            ...
            "y2 lambda Max Effciency vaues regularizer": lambda_y21,
            "y2 lambda different operating points regularizer": lambda_y22,
        ...
        loss2 = loss_reg_y2(lambda_y21, lambda_y22, y2_initial_boundary_threshold, y2_low_nn_threshold, y2_low_mm_threshold)
        ...
        if trainer.val_score_history_y1[-1] < best_y1_score and trainer.val_score_history_y2[-1] < best_y2_score:
            ...
            best_model = model
            best_fold = fold
        model.save(f'./Intermediate/model_fold{fold}.pth')
        wandb.finish()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train MLP model with cross-validation")
    parser.add_argument('--max_torque', type=int, help='Maximum Torque value')
    parser.add_argument('--min_torque', type=int, help='Minimum Torque value')
    args = parser.parse_args()
    main(args.max_torque, args.min_torque)