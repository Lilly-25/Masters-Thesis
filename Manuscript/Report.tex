...
% surements for defining the model parameters.

% e FE-based approach, the computational
...
% 3) To compare ANN and GBDT surrogate model accuracy
...
% volume compared to induction machines. 

% wind power generation, electrical vehicles, and robotics.
...
% rojecting to the 2-D plane of speed and torque, an efficiency
% map consists of varying levels in different regions. The peak
% torque is constrained by the motor’s thermal limit, while the
% maximum speed depends on the rotor’s structure.
...
\begin{enumerate}
    ...
    \item \textbf{Heterogeneous \ac{GNN}} \\    
        Heterogeneous \ac{GNN} are designed for graphs with different type of nodes and edges. 
...
% Given a directed graph \( G = (V, E, X, R, T_v, T_e, \phi, \psi) \), if the graph has a node type mapping function \( \phi : V \rightarrow T_v \) 
% and a link type mapping function \( \psi : E \rightarrow T_e \), where \( T_v \) and \( T_e \) are predefined node type sets and edge type sets, 
% then the network is called a heterogeneous network.

% \( V \) is the set of nodes, with a node type mapping function \( \phi : V \rightarrow T_v \) and 
% \( E \) is the set of edges, with an edge type mapping function \( \psi : E \rightarrow T_e \).

% Each node \( v_i \in V \) is assigned a node type \( c_i = \phi(v_i) \in T_v \) and each edge \( e_{ts} \in E \) 
%  is assigned a relation \( r_{c_t c_s} = \psi(e_{ts}) \in T_e \), pointing from the source node \( s \) to the target node \( t \) \cite{SE HGNN-2023}.
