...
Lastly we will enable the KPI's plot visualisation in a manner presentable to the client Valeo(Automaker Company specialiyes in manufacturing electric motors for cars).\\
...
considered for modelling loss reconstruction nf the model. \\
...
Figure \ref{fig:Standard Deviation of 2D KPI(Efficiency) Targets} shows the standard deviation of few samples of the Torque \ac{KPI} from the entire dataset.
...
Figure \ref{fig:Standard Deviation of 3D KPI(Efficiency) Positive Grid across Speed Intervals} gives us the big picture of how the efficiency values are 
...
This is attributed to the fact we have columns with different ranges for the input since we consider each feature a column mentioned in Section \ref{sec:Deep Dive into Input Parameters}.
...
\begin{enumerate}
    ...
    The number of output features correspond to the target size which in our case is the shape of the padded collated array we had created in Section \ref{sec:Deep Dive into 3D KPI}
\end{enumerate}
...
To smoothen out the curve for the Torque \ac{KPI} we apply the below loss regularisation factor to ensure that the torque values at neighboring operating points are 
...
Based on one of our observation in Section \ref{sec:Deep Dive into 3D KPI} on the efficiency values beyond the Efficiency envelope having blank values, 
...
\begin{equation}
\text{Y2 Maximum Efficiency Regularization} = \frac{1}{n} \sum_{i=1}^{n}\frac{1}{w} \frac{1}{h} \sum_{j=1}^{w} \sum_{k=1}^{h}\left(\text{\ac{ReLU}}(|\hat{y}_{ijk}| - 100)\right)^2 
...
Additionally, to encourage the model to learn the nature of the Efficiency \ac{KPI} from our observations gathered in Section \ref{sec:Deep Dive into 3D KPI} and from 
...
curve in Section \ref{sec:Deep Dive into 2D KPI}.
...
Dropout rate during training is controlled by the parameter \textit{$p_{y1}$} for shared layers and \textit{$p_{y2}$} for layers specific to Efficiency \ac{KPI}. This is discussed in Section \ref{sec:MLP Model}. \\
...
In comparisions to Figure \ref{fig:Standard Deviation of 3D KPI(Efficiency) Positive Grid across Speed Intervals} we infer that as the speed increases the target 
deviations of efficiency values are not so accurately captured by the model.
...
data without having to be retrained. This is defended by Fredrik Johannessen et al in \cite{ML HGNN-2023}
A hallmark of \ac{GNN}s is that they can efficiently learn the architecture parameters from the training data as demonstrated by Damian Owerko et al in \cite{PO GNN-2018}.

\ac{GNN}s cannot be too deep with layers due to the oversmoothening problem. 
...
Studies also tells us that it can be mitigated by including pre-activation residual connections in the network. CITE\\
...
Given a path
\(
P = T_{v1} \xrightarrow{T_{e1}} T_{v2} \xrightarrow{T_{e2}} \cdots \xrightarrow{T_{vk}} T_{ek}
\), 
...
Existing works for instance Xingtong Yu et al in \cite{PR-HGNN-2024} model heterogeneous graphs by splitting the graph into multiple 
...
However Shichao Zhu et al \cite{RSHGNN-2019}'s work does not undertake this methodology and is also independent of metapaths making it effective 
...
In order to combat this, Ziniu Hu et al in \cite{HGT-2022} has used meta relations instead and a unique adjacencz matrix for each edge type.\\
Works by Joshua Melton et al in \cite{MHGNN-2023} also use metarelations to extract soft metapaths and does message passing over multi-hop neighborhood of nodes. \\
Metarelations are most often used in knowledge graphs as it contains richer schema. 
Such meta relations are shallow embeddings and not as deep as the meta paths as clarified by Carl Yang et al in \cite{HGNN-2020}. \\
One such work by Yiling Zou et al in \cite{HNNC-2023} proposes a heterogeneous network node classification model which converts the 
...
Qimai Li et al in \cite{GCN-2018} demystifies the Graph Convolutional Network and addresses the Laplacian oversmoothing concern by self-training or co-training the model.
...
Most of the existing works demonstrated by Carl Yang in \cite{HGNN-2020} also do not consider the edge features in their model. 
However Fredrik Johannessen et al in \cite{ML HGNN-2023} uses edge features as transactions in a financial network to detect cases of money laundering.
...
Additionally Tsai Hor Chana et al in \cite{EHR HGNN-2024} has used heterogeneous \ac{GNN}s to model the heterogeneity and sparsity in Electronic 
...
Justin Gilmer et al in \cite{QC-MP-2017} introduces modelling that makes it capable of learning features of molecular graphs directly and is 
...
Yuxiao Dong et al in \cite{HNRL-2020} review pitfalls in Heterogeneous representation Learning. They also clarify that for metarelations connecting 2 different node types,  
