...
\ac{PMSM}s are largely classified as Surface Mounted Permanent Magnet and Interior Permanent Magnet machines
...
We can observe the relation between the Torque curve and the Efficiency grid. The values within the Efficiency envelope can be understood with the different contour shades.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        Creates a design mesh from the parametric description of the motor's geometric and physical features.
        \item \textbf{\ac{FEA} simulator} \\
        Multiple \ac{FEA} simulations which are by nature Partial Differential Equations(PDE) is done on this mesh.
        \item \textbf{Matlab Script 2} \\
        The outputs from the \ac{FEA} simulations are post processed and the intermediary outputs which are matrices of values are fed to the next stage.
        \item \textbf{Motor Builder} \\
        Several Motor Builder settings are then adjusted to visualize the \ac{KPI} plots in the application.
    \end{enumerate}
    ...
    \begin{enumerate}
        ...
        For training the network, the targets are taken from the \textbf{Matlab Script 2} which also serves as the ground truth represented as a dotted line. This is not needed for inference.
        ...
        The predictions generated by the neural network is post processed to be similar to the targets of the \textbf{Matlab Script 2} in terms of dimensions.
    \end{enumerate}    
    \item \textbf{KPI Plots} \\
    The targets which are vectors of numerical values are displayed graphically and visualized.
\end{enumerate}
...
Vivek et all in \cite{VAE-MT-2021} also highlights that predicting \ac{KPI}s with tabular data is significantly more efficient than using the images because the latter could result 
...
Similarlz, Yusuke et al in \cite{PANN-MT-2021} and \cite{PANN-MOO-2021} also claim that a physics assisted neural network significantly improved the accuracy performance when predicting the cogging torque 
...
input to the neural network when making the final prediction. With this methodology, they also disclose that a significant less data than estimated suffices for training the neural network. \\
...
Meaning the values only within the torque curve i.e., excitation points are predicted. This results ni conserving training time when predicting a fixed sized grid
...
Secondly, for efficiencies being predicted outside the excitation points, the authors propose to provide an uncertainity measure to quantify confidence level using Monte Carlo dropout.
...
\begin{enumerate}
    ...
    The 2 Linear Layers enable the network at the start to learn a rich representation of the data at the initial feature extraction phase.
...
Tabular representation of data in itself cannot account for the spatiality of an \ac{EM} design especially on how different components interact with one another.
...
\ac{GNN}s in general function by making use of \ac{MP} which is a recursive algorithm. \ac{MP} is based on the graph structure and initial node features.
Over each hop of successive neighbourhoods in the graph, the information of neighbouring nodes of a node are shared across as messages and aggregated 
and the initial node is recursively updated with this information. This continues until there are no more nodes to traverse within the graph.
...
On the basis of the directionality of edges within the graph, graphs in general can have the following taxonomy: \cite{GNN-2019}:
\begin{enumerate}
    \item Directed Graph - A graph in which the edges have a direction from source node to its target node.
    \item Undirected Graph - A graph  Adjacency matrix is symmetrical such that for each edge there will be its respective reverse edge in the opposite direction.
\end{enumerate}
...
than transductive. While transductive model can only be used on the specific data that was present during training, 
...
\begin{enumerate}
    ...
    \item \textbf{Heterogeneous \ac{GNN}} \\    
        Heterogeneous \ac{GNN} are designed for graphs with differing type of nodes and edges. 
        Different types of nodes and edges also imply difference in features and dimensionality.
...
\), 
if the path describes the composite relationship \( R_1 \circ R_2 \circ \cdots \circ R_l \) between nodes of type \( T_{v1} \) and \( T_{vl+1} \), then the path is 
called a metapath. The metapath is also the sequence of many metarelations.\\
...
Graph isomorphism is the property of graph being similar even when the ordering of nodes and edges are different.
...
\begin{thebibliography}{00}
    ...
    \newblock Yusuke Sakamoto, Yihao Xu, Bingnan Wang, Tatsuya Yamamoto, Yuki Nishimura
    \newblock {\em Multi-Objective Motor Design Optimization with Physics-Assisted Neural Network Mode}.
    \newblock IEMDC, 2023.
