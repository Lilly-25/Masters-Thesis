...
(PMSM) Electric Motors (EM)s from its geometric, physical and simulation parameters.
I intend to model the dynamics of EMs functionality by creating surrogate models trained with Finite Element Analysis (FEA) simulated KPIs and its parametric description.
...
I note the relationship between the Torque curve and the Efficiency grid and incorporate my learnings of both the KPIs nature into the solutions modelling.
First, I parameterize the EM design such that it is feasible to convert into a tabular representation.
Next, I design a Multi Linear Perceptron (MLP) to train it on the tabular representation of the data in a supervised manner.
Subsequently, I regularize the loss functions in a way that would smoothen out the plots for both the KPIs. 
I note that the loss regularized by taking into consideration the Efficiency grid's nature marginally improved the predictions of the Efficiency grid as opposed to having 
no loss regularizations.
Then, I evaluate the predictions with the test target values by experimenting with various hyperparameter settings and as a baseline with the average of the labels in 
the training dataset. I also conduct comprehensive ablation studies to showcase the performance of my models with that of the \ac{FEA} solver and comment on the substantial
improvement in time complexity.
Additionally, I conduct a case study to model the task as its graph representation and discuss the possibility of training Graph Neural Network (GNN)s to predict the KPIs.
Finally, I conclude with my findings and reflect on the decisions I had made during the course of the thesis offering fresh perspectives for future work.
...
Special thanks are in order to Daniel and Leo for sharing valuable insights of the data from an electro-mechanical standpoint and for giving me a detailed understanding 
...
which are essentially \ac{PDE}s.\\
...
The paper in Ref. \cite{ETA-2021} also states that the Efficiency vs Torque and Angular Velocity map is most representative of the characteristics of the \ac{EM}.  \\
...
phase. This enables designers to estimate the performance and make a well informed decision on manufacturing it. Although \ac{FEA} only gives approximate solutions, 
...
The coloring of the image possibly resembled the intensity of magnetic flux density or electric current distribution flowing across different parts of the \ac{EM}.\\
...
generate \ac{EM} efficiency maps in terms of accuracy by experimenting with overall error rates across operating ranges for different \ac{EM} losses.\\
...
Deep learning can be seen as the most eligible surrogate of \ac{FEA} simulator as it can provide almost accurate results in relatively no time. 
...
\begin{figure}[H]
    ...
    \caption{\ac{EM} Motor (Source : \href{https://www.valeoservice.in/en-in/newsroom/basic-understanding-automotive-electric-motors}{https://www.valeoservice.in/en-in/newsroom/basic-understanding-automotive-electric-motors})}
    \label{fig:Valeo Motor Structure}
\end{figure}

The actual engine data of \texttt{Valeo}\footnote{\url{https://www.valeo.com/}} is used here as the dataset comprising of multiple design variants of the Double-V 
...
\begin{figure}[H]
    ...
    \end{minipage}
\end{figure}
Fig. \ref{fig:Torque Curve} and Fig. \ref{fig:Efficiency Grid} give a visualization of the Torque curve and the Efficiency Grid of a \ac{EM} design variant respectively 
...
as being inversely proportional to the Efficiency values.
...
Companies could then evaluate the \ac{KPI}s of the synthetically generated \ac{EM} designs and judge its usefulness. 
...
I share all supplementary information in the Appendix section.
Lastly, Bibliography section lists out the articles cited for this thesis. 
...
\subsection{CNN vs MLP}\label{subsec:LR CNN Vs MLP}

There has been extensive research on modelling the \ac{EM} with \ac{CNN} based on the images of the motor cross-section. Reproducing images is not the most wisest 
...
The auxiliary input is supplemented by \ac{FEA} simulations which are enriched with the nature and placement of 
...
The authors also claim to have improved the generalization performance of \ac{CNN}s with this strategy. In addition, they suggest such domain knowledge modelling decisions 
can be beneficial for both parameter and topology optimization of \ac{EM} as well as for the prediction of other \ac{KPI}s.\\
...
using images because the latter could result in less accurate designs due to the need of high resolution images in addition to its overall computation cost 
...
The writers argue that although there are classical machine learning models which handle multi regression, they do not perform very well with higher dimensions as opposed to deep neural 
networks. They also point out that these models fare better than deep neural networks when it comes to data required and training time.\\
...
and contrasts the computational efficiency and accuracy with regards to both approaches. \\
...
performance when predicting the cogging torque of \ac{PMSM} Motors. Their experiments involve approximating the cogging torque with a 
linear subdomain model which serves as an additional input to the neural network when making the final prediction. With this methodology, they also disclose that 
significantly less data than estimated would suffice.
...
to each other. This eventually assists the model to generalize better and conserve the time it would have spend training from scratch by reusing knowledge. 
...
was discussed in Section \ref{sec:Research Question}. The latter could potentially be seen as a future work since the time complexity associated with the 
...
within the torque curve i.e., the excitation points are predicted. This further results in conserving training time when predicting a fixed sized grid.
They also use confusion matrix for efficiency threshold so motor designers can filter out efficiencies in the operating range one is most eager to find out.\\
...
Monte Carlo dropout. The rationale behind it is that the error rate in the efficiency grid is maximum at the envelope of the curve. This factor gives the end user 
...
Valeo has shared 1481 Excel Workbook files for each variant of three-phase \ac{PMSM} \ac{EM} with 8 poles and 48 slots which are the 6 stator slots per cross-section. 
I detail out the structure of the dataset files in Appendix \ref{subsec:File structure} with each sheets usage and role for visualizing the \ac{KPI}s.\\
...
The parameters of the \ac{EM} are supplied for 1 slice which can be simulated to model the full \ac{EM} for \ac{FEA} simulations. Similarly, for surrogate modelling a 
slice of the \ac{EM} is sufficient to predict the \ac{KPI}s else it results in a representation of redundant information without any variability as such.\\
\begin{figure}[H]
    \centering
...
I have also included a hand drawn sketch from my understanding of how the geometry of 1/8th cross-section of the same \ac{EM} looks like in Fig. \ref{fig:1/8 Motor Crossection}.
This particularly comes in handy when creating the graph representation of the \ac{EM} in Section \ref{subsec:EM Heterogeneous Graph Construction}.
...
up of copper reside at. The Stator yoke separates each tooth shoe which is vaguely the outer part of the stator slot from the outer radius of the \ac{EM}.
...
However, I focus only on the Double V Magnet topology for data exploration of the \ac{KPI}s as well as the dataset which I use throughout the thesis for training and 
...
variations in parameter values are significant enough to be considered as a fundamentally different \ac{EM} design.\\
For the rotor parameters in Fig. \ref{subfig:Rotor Parameters}, the variance is reported to be the least with comparably 14 parameters showing a slight deviation when 
contrasted with the 70 parameters the rotors across the 3 topologies possess. Meanwhile for the stator parameters in Fig. \ref{subfig:Stator Parameters}, the variance is 
...
compared to the other 3 parameters. \\
The difference in topologies are largely visible in its rotor components in terms of new parameters being introduced. However, no additional parameters crop up in the 
stator and general parameters with different topologies.
...
However, the imbalanced topologies parameters would have relatively not so reliable statistics as only 3 variants each where considered.
...
From my understanding of which parameters are most sensitive to the \ac{EM} design, I have chosen 89 parameters which comprises of the geometric, physical and 
simulation properties of the motor out of a total of 253 parameters across the 3 topologies depending on its overall variability and significance.
The electrical parameters such as voltage, current in addition to magnetic properties such as type of magnet among many other are excluded.
I focus largely on the parameters accounting to the geometry of the \ac{EM} as I believe its structure is of paramount importance to generate \ac{KPI}s 
whereas the electrical and magnetic parameters are essential for generating simulations of \ac{EM} using the \ac{FEA} software. 
This was a design decision I had made based on my understanding of the data.\\
Fig. \ref{fig:Proportionality in Parameter Selection} shows the proportionality in parameter selection across the 3 topologies.
The selected parameters account for 35.2\% of the total parameters and are depicted in green when compared to the unused 64.8\% of the parameters in grey.

\begin{figure}[H]
    \centering
    \includegraphics[width=.35\textwidth]{./ReportImages/params_proportionality.png} 
    \caption{Proportionality in Parameter Selection}
    \label{fig:Proportionality in Parameter Selection}
\end{figure}
...
a relatively narrow scale. The range in variance of the other parameters as was evident in Fig. \ref{fig:EM Parameter variance Analysis} is already significantly low.
I assumed radian conversion would be relevant given the variety of shapes in the \ac{EM} design such as the air pockets and the fact that the designs are of 6 phase 
\ac{EM} variants which implies phase shifted sinusoidal waves.
...
The x-axis represents the angular velocity of the \ac{EM} ranging from 0 to 19100 rpm. Meanwhile, the y-axis represents the torque values corresponding to the 
...
Furthermore, I display a twin y-axis in red showing the standard deviation of the samples with the mean in dotted lines.\\
...
\begin{enumerate}[nosep]
    ...
    saturation is visible with the plateauing of the curve until the standard deviation touches close to 1.
    \item \textbf{The curve to an extent resembles a Sigmoidal function mirrored along the x-axis}\\           
    This finding is critical for how I model the loss regularization for the Torque \ac{KPI} and will be further elaborated in Section \ref{sec:Loss for 2D KPI}.
    \item \textbf{Samples are almost similar to one another}\\
    The samples shown here are almost resembling one another in the shape and nature of the curve. The finer details are at the points of the curve where it makes its 
    transitions. This observation is crucial and directly impacts my decision on choosing the Baseline model to be discussed in greater detail in Section 
    \ref{sec:Results with Baseline}.
    ...
    prominent on visualizing the results of the Torque curves and comparing the \ac{RMSE} values in Sections \ref{subsec:Torque KPI Results with MLP Efficiency KPI Regularization}, 
    \ref{subsec:Torque KPI Results with Baseline}, \ref{subsec:KPI Results with Smoothening curve Loss Regularization},
    \ref{subsec:KPI Results with Decreasing curve Loss Regularization} and \ref{subsec:Torque KPI Results with No Loss Regularization}.
    ...
    modelling phase and will be discussed in Section \ref{sec:Loss Functions} how I use it in teaching the model.
\end{enumerate}
...
Subsequently, I choose only the rows of the MM grid introduced in Table \ref{tab:Excel File Structure} as the angular velocity which correspond to the indices of the 
sliced efficiency grid.
...
of the efficiency values with the torque values displayed against its angular velocity considering all samples of the dataset. The standard deviation levels are shown 
as a scale with colors darkening as the standard deviation increases.\\
...
monitoring mode. In both modes, the efficiency grid are supposed to almost resemble a mirrored image of one another.
...
\begin{enumerate}[nosep]
    ...
    However over the course of this thesis, I do not do so as it is irrelevant to evaluate a duplicate prediction.\\
\end{enumerate}
...
This is attributed to the fact that I have columns with fundamentally different ranges for the input since I consider each feature a column as mentioned in 
...
\begin{enumerate}[nosep]
    ...
    \item Scaling is beneficial when the targets are of different scales as is mine. A way to handle differing scale of targets would be to use the MinMax Scaler to 
    ...
    \begin{enumerate}[nosep]
        ...
        \item Additionally, if I scale the targets, changes will need to be made accordingly for the loss regularization which I model in Equation 
        \ref{eq:Y2 Maximum Efficiency Regularization}. It gets tricky as I assume maximum efficiency that can be generated by a vehicle 
        is 100\% whose scaled version I do not know since the maximum efficiency value from the training dataset is around 98\%.\\
...
Since this comes at a tradeoff of potentially losing precision and possible uncertainty of the nature of the test dataset, I continue with the original targets as is.
...
I have also partitioned the dataset to have about 50 samples for test and the remaining to be used for 5 fold cross validation with 80:20 split for training and validation. 
...
Therefore, I expect to cover most grounds on training and have good monitoring of the model's performance.
As a matter of fact, cross validation enables one to be able to monitor the network's overall stability and thus validate the model's generalization performance.\\
...
\section{MLP Model}\label{sec:MLP Model}

For my multi-regression problem, I make use of a \ac{MLP} model with input features corresponding to all the features in the tabular topology-invariant representation of 
the data. The model architecture is build to predict both the Torque \ac{KPI} and Efficiency \ac{KPI} by having 2 separate output layers for each of the \ac{KPI}s. 
...
Thus, both batch normalization and dropout layers stabilize the network's training.\\
...
The \acl{MSE} (\ac{MSE}) loss is the error metric used for the problem with the intention that the squared losses penalizes the model and inturn encourages it to 
...
Based on one of my observation that the Torque curve closely resembles a mirrored sigmoidal curve, I penalize the loss for increasing consecutive 
...
Moreover, these predictions are irrelevant for us because after generating all predictions I finally slice off the Efficiency grid to be of the shape of the Torque curve 
which entails that the values predicted in place of \ac{NaN} are irrelevant. This is discussed more elaborately in Section \ref{sec:Post Processing}.
...
The evaluation metrics regarded for the regression problem is the average of the \acl{RMSE} (\ac{RMSE}). 
Therefore, the model with the least prediction scores such that it is closest to 0 is the ideal score. 
Although, \ac{RMSE} is used as the metric for evaluation being very similar to \ac{MSE} loss functions, the loss regularizations play a vital role on the loss functions on top of \ac{MSE}.\\
Needless to mention, I have the Masking mechanism as per Equation \ref{eq:Mask matrix} for the portion outside the envelope of the Efficiency \ac{KPI} which does not exists for the evaluation function of the 
...
In the case of new files not part of the segregated test dataset, I first convert it into tabular representation that the model consumes and then apply scaling.
...
I also would like to point out that the 2 outputs typically would thrive on different learning rates and it might be a better approach to have separate learning rates in addition 
to separate loss functions.\\
The \textit{batch size} is limited to the capacity of the \acl{GPU} (\ac{GPU}) memory. I use the maximum batch size to train faster after which I hit the memory roof of the \ac{GPU}.\\
...
Dropout rate during training is controlled by the parameter \textit{$p_{y1}$} for shared layers and \textit{$p_{y2}$} for layers specific to Efficiency \ac{KPI} which 
is discussed in Section \ref{sec:MLP Model}. I am not so much concerned of the data being dropped in larger extent for the Efficiency \ac{KPI} than anticipated 
...
regularization terms of the Torque \ac{KPI} and Efficiency \ac{KPI} respectively detailed in Equation \ref{eq:Total Loss}.
...
Fig. \ref{fig:Validation Evaluation Metrics} illustrates the \ac{RMSE} evaluation metrics changing across each epoch of the validation dataset. This is important to us to know 
...
The Torque \ac{KPI} evaluation metrics start at a score of 160 and converge to around 7 by the 7th epoch. Meanwhile, the Efficiency \ac{KPI} evaluation metrics starts off at 
...
\begin{enumerate}[nosep]
    \item The difference in the losses and scores at which training and validation begins for both \ac{KPI}s can be attributed to larger value range for 
    ...
    range ($n^2$ values being a matrix).
...
The model I have used here is the \ac{MLP} model with the Efficiency \ac{KPI} loss regularization terms.
...
displayed against their angular velocities. The difference and percentage difference between the prediction and ground truth are also displayed on a twin y axis in dotted 
lines to give a rough overview of the prediction deviations from the targets. As the percentage difference formulated in Equation \ref{eq:Y1 Percentage Difference} would 
...
Furthermore, I display a twin y-axis in red showing the \ac{RMSE} with that of the ground truth in dotted lines.
...
\ref{fig:Y1 Percentage Difference}. The latter is essential to be able to relate to its effect on total range of deviation due to its differing value range. 
...
\begin{enumerate}[nosep]
    ...
    that the torque values have smaller values than its respective ground truths. It is evident from difference shown above the dotted line. 
    However, most of the examples show very close correlation between prediction 
    ...
    mean distribution. 
    \item Additionally, the \ac{RMSE} is shown to be fluctuating. Experimenting with the hyperparameters 
...
The figure shows the mean Efficiency values and the \ac{RMSE} surrounding it as pink shaded overlap for the 10 handpicked samples from the test dataset.
The angular velocities are chosen at equal intervals of 2000 rpm. For each of the angular velocities, the range in participating torque values differ implying the 
envelope of the Efficiency grid. The efficiency values as usual range between 0 and 100\% whereas the torque values are shown 
from 0 to approximately 250 Nm which could be the maximum torque value from the predictions of the samples.
The \ac{RMSE} of each prediction from its target can be viewed on the twin y-axis ín red as dotted lines.
...
and \ref{eq:Y2 Percentage Difference} corresponding to \ac{RMSE} and $\mathcal{Y}_2$ percentage difference as histogram plots over the entire test dataset of 50 samples 
with the count of samples on the y-axis.
...
of all folds against its torque values for different angular velocities. Once again, to overcome the limitation of not being able to evaluate for all angular velocities, 
the Figure hosts only 6 subplots for specific angular velocities. The evaluations is for the same target used in Fig. 
\ref{fig:MLP Training Deviation across Folds for Torque KPI} for all of the 5 folds.
...
\begin{enumerate}[nosep]
    ...
    \item Furthermore, it can also be noted the efficiency values are being predicted well from the contours of the Efficiency grid.
    ...
    \item In addition, Fig. \ref{fig:Efficiency KPI Predictions Vs Targets with MLP} and Fig. \ref{fig:Efficiency KPI Predictions Vs Targets Vs Difference with MLP} would also 
    ...
    \item Another observation that can be made is that the average \ac{RMSE} overlap along the Mean Efficiency is at its peak towards extreme torques across the 
    ...
    \ref{fig:MLP Standard Deviation of 3D KPI(Efficiency) across Angular Velocity Intervals with MLP} that the efficiency values do not exceed 100\% which makes the 
    effect of the regularization in Equation \ref{eq:Y2 Maximum Efficiency Regularization} more evident.
...
Fig. \ref{fig:Baseline RMSE Evaluation for 2D KPI(Torque)} shows the Average \ac{RMSE} and element wise \ac{RMSE} for the test dataset with the 
...
The model I have used here is the \ac{MLP} model with Torque \ac{KPI} loss regularization terms.
As I am only regularizing the Torque \ac{KPI}, I have refrained from evaluating the Efficiency \ac{KPI} here.
...
Fig. \ref{fig:Smoothening Torque RMSE Evaluation for 2D KPI(Torque)} shows the average \ac{RMSE}, mean and element wise \ac{RMSE} for the 10 samples from the test dataset 
...
\begin{enumerate}[nosep]
    ...
    2.5.
    \item Apart from the 2 samples which I expect the \ac{RMSE} to be high owing to them being outliers as was discussed in Section \ref{subsec:Deep Dive into 2D KPI}, 
...
Fig. \ref{fig:Decreasing Torque RMSE Evaluation for 2D KPI(Torque)} shows the mean, average \ac{RMSE} and element wise \ac{RMSE} for the 10 samples from the test dataset 
with the model. The structure of the figure is the same as Fig. \ref{fig:MLP RMSE Evaluation for 2D KPI(Torque)}.
...
\begin{enumerate}[nosep]
    ...
    ranging upto 2.
    \item Apart from the 2 samples which I expect the \ac{RMSE} to be high owing to them being outliers, 3 additional samples appear to also have a high \ac{RMSE}.
    \item The deviations are at its peak towards the beginning of the curve.
\end{enumerate}

\section{Results with MLP No Loss Regularization}\label{sec:Results with No Loss Regularization}
The model I have used here is the \ac{MLP} model without any loss regularizations.

\subsection{Torque KPI Results}\label{subsec:Torque KPI Results with No Loss Regularization}

Fig. \ref{fig:No Loss Regularization RMSE Evaluation for 2D KPI(Torque)} shows the mean, average \ac{RMSE} and element wise \ac{RMSE} for the 10 samples from the test dataset 
...
\begin{enumerate}[nosep]
    ...
    \item Apart from the 2 samples which I expect the \ac{RMSE} to be high owing to them being outliers, the other samples appear to be performing well in terms 
...
I conduct comprehensive ablation studies on the \ac{MLP} model with different loss regularizations and the Baseline model for both the targets.
...
Additionally in Appendix Fig. \ref{fig:KPI Predictions for all Models for the same sample}, I generate \ac{KPI}s of 1 sample for all the models I developed to 
...
\begin{enumerate}[nosep]
    ...
    than the actual values. This prompts us to exclude the Torque \ac{KPI} regularization from the main model.
...
\begin{minipage}[t]{\textwidth}
    \begin{table}[H]
        ...
        \hline 
        Surrogate Modelling (Proposed Approach) & 18.5 seconds & 16 \ac{CPU} Cores \& 1 \ac{GPU}\\
        \ac{FEA} simulations (Current Approach)\footnote{Source : Valeo} & 5 minutes & 1500 \ac{CPU} Cores\\
        \hline
        \end{tabular}
...
\ref{fig:EM Design Flowchart} is only possible by running the \ac{FEA} simulator across High Performance Computing machines of nearly 1500 \ac{CPU} cores in parallel. 
As the number of \ac{CPU} cores decrease the simulation would take minutes to hours to converge. Needless to mention, it is the \ac{FEA} Solver in the block that is the culprit for 
...
The compute time of the Proposed Approach is approximately 16 times faster than the Current Approach even without considering the disparity in hardware used.
As opposed to \ac{FEA} simulations, the time required to generate predictions for several motor designs is memory bound 
and computation is not so much of a bottleneck since I can potentially run both training and inference across multiple \ac{GPU}s in parallel.\\
...
The model was trained on a NVIDIA Tesla V100 \ac{GPU} with 32 GB Memory and the machine I used had 16 physical \ac{CPU} cores of the Intel(R) Xeon(R) Silver 
4208 \ac{CPU} operating at 2.10 GHz with allocated disk space of 50 GB for my experiments.\\
Source code is made available at \texttt{\href{https://github.com/Lilly-25/Masters-Thesis}{Github Repository}}\footnote{\url{https://github.com/Lilly-25/Masters-Thesis}}.
...
Researchers have broadly classified \acl{GNN} (\ac{GNN})s into the following 2 distinct paradigms:
\begin{enumerate}[nosep]
...
All knowledge graphs are heterogeneous graphs but all heterogeneous graphs are not knowledge graphs as is my case.\\
...
I refer to Section \ref{subsec:GNN Terminologies} where the terminologies in \ac{GNN} are more elaborately defined.\\
...
\begin{enumerate}[nosep]
    \item Graph Classification - Used for classifying graphs into different categories. Although my usecase is a graph level prediction, it is a graph multi-regression 
    task as I need to predict continuous values as was clarifed in Section \ref{sec:Objective}
    ...
    \item Graph Generation - Generates synthetic graphs based on initial graph representation. An application I can relate to is the inverse formulation of my problem 
...
\begin{figure}[H]
    ...
    \caption{\ac{GNN} Tasks (Source : \href{https://www.datacamp.com/tutorial/comprehensive-introduction-graph-neural-networks-gnns-tutorial}{https://www.datacamp.com/tutorial/comprehensive-introduction-graph-neural-networks-gnns-tutorial}) }
...
After which I present the methodology in Section \ref{sec:EM Heterogeneous GNN Model} and share my view on how to create the heterogeneous graph for my usecase.
...
Fig. \ref{fig:MetapathNeighbors} illustrates the neighbors of the nodes in the metapath and Fig. \ref{fig:MetapathNeighbors} connects the neighbors of a particular node 
to form its subgraph. \\

\textbf{Metarelation}

For an edge $e = (s, t)$ linking source node $s$ to target node $t$, its meta relation is denoted as $(\phi(s), \psi(e), \phi(t))$.
...
increase exponentially with the number of node and edge types. Hence, the reason to choose only few metapaths that are significantly most relevant for the task at hand. 

Metapath selection requires domain knowledge to be able to choose those paths which are most semantically meaningful and let the Heterogeneous \ac{GNN} use only them 
...
node $v_1$ along the metapath $\mathcal{P}$ are defined as the set of nodes connected to $ v_1$ along the meta-path $\mathcal{P}$ which includes $v_1$. \\
...
function convolves over each of the aggregated messages. Finally, all messages generated after final convolution which are of the same dimensions are once more aggregated 
...
multilayer smaller metapaths to capture semantic information of the graph. Instead of attention mechanism of neighboring nodes repeatedly, the authors propose to 
compute the mean aggregate of the neighborhood to capture the structural information as part of a preprocessing step thus simplifying the heterogeneous \ac{GNN}. \\
...
homogeneous subgraphs based on their node types, each subgraph retaining an aspect of its heterogeneity.
...
heterogeneous network into multiple semantic graphs from its metapaths. Attention mechanism is leveraged to learn the weight of each of the subgraphs. \\ 
...
\ac{GNN}s often face a dilemma to balance between resisting oversmoothing and capturing long range dependencies within the network.
...
Nonetheless, smoothing makes classification problem easier because over multiple hops the features among different clusters will be the same.
...
They break down the large heterogeneous graph into subgraphs based on different node type-edge type combinations. \\
...
heterogeneous graph.\\
Dong \textit{et al.} in Ref. \cite{HNRL-2020} review pitfalls in Heterogeneous graph representation Learning. They also clarify that for metarelations connecting 2 different 
...
Alternatively, Heterogeneous Graph Transformers can be used to learn the implicit metapaths from the graph by using feature propagation across multiple layers in its 
...
Although I was certain that \ac{GNN}s could best represent my usecase, I realize that the problem cannot be solved using Homogeneous \ac{GNN} which is relatively 
simpler and is built on a single node and edge type.
Heterogeneous graph is more apt for my use case with its different node and edge types as it preserves both the structural and semantics of our data. 
This property is crucial in modelling the use case as there will then be similar number of nodes and edge per topology. 
...
$\mathcal{H}$ be the 2 \ac{KPI}s that needs to be learned from $\mathcal{D}$ then the future aim would be to train a multi-task \ac{GNN} model $\mathcal{M}$ such that $\mathcal{M}$ can deliver 
...
Inspired by the promising advantages of Heterogeneous \ac{GNN}, I have taken the effort of constructing the graph for only the Double V Magnet Topology.
I create the heterogeneous graph to be adaptable for each of the \ac{EM} variants as a \texttt{NetworkX}\footnote{\url{https://networkx.org/}} graph.\\
...
to rotor structure. \\
The hand drawn sketch of the \ac{EM} cross-section comes in handy when creating the heterogeneous graph as shown in Fig. \ref{fig:HetGraph}. 
In Table \ref{tab:GNN Node Types} and Table \ref{tab:GNN Edge Types}, I illustrate the nodes and edges along with its respective types which I had considered when 
creating the graph. I also share the metapaths that could potentially be used for further development and would like to emphasize that this is a study and its results 
are not to be taken as final. Finally with the heterogeneous graph, a heterogeneous \ac{GNN} model $\mathcal{M}$ could be developed using 
\texttt{Pytorch Geometric}\footnote{\url{https://pyg.org/}} with cuda.

\ac{GNN}s are relatively more expressive models as they can capture the variability and complexity of my dataset better.
...
I refer to Section \ref{subsec:EM graph construction} where I elaborately discuss how I have created the graph.
My case study is loosely inspired from Ref. \cite{ML HGNN-2023} which has strived to extend \ac{MP} into Heterogeneous \ac{GNN}s.

\section{Summary}\label{sec:EM Heterogeneous GNN Discussion}
Heterogeneous \ac{GNN}s are a relatively new concept and there are fewer publications to derive meaningful insights from. This has made it challenging to understand as 
there are hardly any implementations one can intuitively refer to. Nevertheless, even if we model the usecase as a Heterogeneous \ac{GNN}, there lies the possibility that 
computation could be expensive. The rationale behind as was already discussed is that \ac{MP} is done across each node and edge type. The added complexity is in defining meaningful 
metapaths that are meaningful to the task and its possibility of being flexible to changes when completely different topologies are intorduced. In hindsight, it could 
also be interesting to model the Heterogeneous \ac{GNN} without metapaths as to my understanding metapaths are generally defined for 1 large graph and not multiple small ones like ours.\\
The advantages of modelling with the Heterogeneous \ac{GNN} is its ability to be adaptable to different topologies when it comes to the structure of the graph.
Additionally, the long term goal of generating the inverse formulation of the problem as was discussed in Section \ref{sec:Research Question} with graphs is interesting 
as well as promising.\\
I have not had the time to implement the Heterogeneous \ac{GNN} model during the stipulated time of my thesis. However, I have shared my thoughts on how to go about 
and would be interesting to witness it implemented.
...
I have developed a topology invariant \ac{MLP} model to predict 2 \ac{KPI}s of an \ac{EM} from its parameteric description. 
...
superior run time efficiency. The latter can be inferred from Section \ref{sec:Time Comparison}.\\
...
to my usecase, to my knowledge, there has not been any reliable implementation that can be reused or benchmarked against. Therefore, what makes the thesis stand out is 
that it has been developed from scratch and can be easily modified to reproduce the work or similar works.
I would also like to note that incorporating the nature of the \ac{KPI}s particularly the Efficiency \ac{KPI} into the loss functions significantly helped the model generate 
...
I have also debated my perspectives on scaling the targets in Section \ref{sec:Scaling} but would consider possible alternatives to counter the concerns I had highlighted.\\
I would like to stress that my experiments are conducted on a relatively small dataset and the approach has potential to work better with a larger variety of data.

\section{Future Improvements}\label{sec:Future Improvements}
I would suggest the following improvements to my thesis : 

\begin{enumerate}[nosep]
    \item Build a model that predicts both the \ac{KPI}s by utilizing the Torque \ac{KPI} prediction to predict its corresponding Efficiency \ac{KPI}. 
    It can be typically visualized as a model with complex outputs where one is dependent on the other.
    \item Another interesting study would be to benchmark model performance when one backpropagates the losses for each \ac{KPI} individually rather than aggregating and 
    weighing them.
    \item Explore different learning rates specific to each \ac{KPI} would also potentially aid in balancing the importance of the 2 \ac{KPI}s.
    \item Scale the targets to be of the same range to combat significantly different value ranges overcoming the forseen setbacks.
    ...
    \item Additionally, the motivation of building a topology invariant model was the reason I have considered building a heterogeneous graph to model the data. 
    The machinery of creating the graph is elaborated in Section \ref{sec:EM Heterogeneous GNN Model} and Appendix \ref{subsec:EM graph construction}.
...
Needless to mention, the imbalanced topologies parameters typically in the rotor do not have reliable statistics.\\ 
...
The csv files are then concatenated and stored into an array conserving dimensionality by padding \acl{NaN} (\ac{NaN}) values to match dimensionality of the grid 
corresponding to the Torque \ac{KPI} with the largest torque value.
In my case, the value is 280 computed internally from the dataset but this is subject to change as I receive more data and there is a provision for the client to override it 
...
However, Fig. \ref{fig:KPI Predictions for all Models for the same sample} evaluation scores indicate that its predictions are misinterpreted by being within the range 
...
I very largely adopt the commonly used notations from Ref. \cite{GNN-2019} and reformulate it slightly for my requirement.\\

\textbf{Graphs}

A graph is represented as \( \mathcal{G} = (V, E) \) where \( V \) is the set of nodes  and \( E \) is the set of edges. 
...
\( x_v \in \mathbb{R}^d \) representing the feature vector of a node \( v \) where $n$ is the number of nodes and $d$ the dimensionality of the node feature vectors. 
...
where $m$ is the number of edges and $c$ the dimensionality of the edge feature vectors.\\
...
The Graph Laplacian is defined as \( L := D - A \) and \\
the Normalized Adjacency Matrix can be defined as
\[
...
The Transformation Operator is a feature map or a learnable matrix of weights, \( \theta \in \mathbb{R}^{F \times F'} \) where $F$ is the dimensionality of the input feature vector.\\
...
It is also called a Polynomial Graph Filter or Radial Filter and can be expressed as $V' = \sigma (\sum_{k=0}^{K}R^{k}V\theta^{k})$ where $K$ is the size of the kernel.
...
\label{subsec:EM graph construction}

We detail the construction of the NetworkX Heterogeneous graph for the \ac{EM} dataset based on Fig. \ref{fig:1/8 Motor Crossection}.
All decisions of $V, E, X, R, T_v, T_e$ from Section \ref{sec:Heterogeneous GNN} are concluded from Fig. \ref{fig:1/8 Motor Crossection}.\\
...
\begin{enumerate}
    ...
    \end{table}
\end{enumerate}
The edge types are distinct according to its dimensionality of features and its semantic meaning. From the semantic perspective, I have 2 edge types: distance and angle 
whereas dimensionality wise, I have 3 edge types of semantic value distance. These 3 distance edge types are further classified on the basis of number of features it 
possesses as 1, 2 and 4 features respectively.\\

As graph attributes, the shaft diameter parameter $\phi wd$ from Fig. \ref{fig:1/8 Motor Crossection} can be supplied. However, I have not considered this parameter 
when modelling the \ac{MLP} as there was 0 variablilty in the dataset received. Besides this parameter, the targets from Table \ref{tab:Excel File Structure} Mgrenz and ETA can 
be assigned as separate graph attributes since the problem needs to be modelled as a graph level prediction.\\

The metapaths that could be defined for the graph are as follows :\\
Rotor Metapath, $\mathcal{P}_{1} = {v, v, r, r} $\\
Stator Metapath, $\mathcal{P}_{2} =  {r, r, s, sw, s, r} $\\
Full \ac{EM} Metapath, $\mathcal{P}_{3} = {v, v, r, r, s, sw, s, r} $\\
However, the metapaths selected are purely hypothetical and needs to be validated with a electromechanical expert on the flow of magnetic flux within \ac{EM} before it 
can be used for training.\\

Fig. \ref{fig:HetGraph} resembles the heterogeneous graph I have created from Table \ref{tab:GNN Node Types} and Table \ref{tab:GNN Edge Types} with 5 node types and 
4 edge types. The nodes of the graph are depicted as the nodes and the edges are labelled as their edge type and not edge name for brevity.
It represents the heterogeneous graph in a hierarchical ordering of nodes such that the edges overlap among each other the least.
Despite the hierarchical view, edge $e_{o,ra}$ does overlap with edge $e_{o,ri}$ however the edge type remains the same for both edges which is $d1$.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{./ReportImages/DoubleVGraph.png} 
