...
In the design of \ac{EM}, vast amounts of data are generated to determine which design of an \ac{EM} fits best to \ac{KPI}s.
...
I can observe the relation between the Torque curve and the Efficiency grid. 
...
Most importantly I can also make out that the Efficiency envelope is of the same shape as the Torque curve.
...
I aim to approximate the targets by training a \ac{MLP} model $\mathcal{M}$ to learn the mapping between $\mathcal{X}$ and $\mathcal{Y}_1$, $\mathcal{Y}_2$.
...
\begin{enumerate}[nosep]
    ...
    Given my noted observation that the efficiency envelope is controlled explicitly by the torque curve, I discuss in Section \ref{sec:Post Processing} how I tackle this scenario.
    There has been literature on modelling wherein the values of a target are dependent on the values of another target. However, the situation is tricky in the sense 
...
In the Dataset section a detailed insight is elaborated on how the data is structured .
In the Modelling section, I introduce the network architectures and loss regularization techniques used to tackle the problem.
The outcomes of the work are presented in Experiments and Results chapter in addition to other findings I unearth.
Graph Modelling section is an empirical study which outlines the background of \ac{GNN}s primarily Heterogeneous \ac{GNN}s and defines its concepts. I also attempt to 
...
I review the concept of surrogate modelling and its applications in \ac{EM} domain.

\section{Surrogate Modelling}\label{sec:Surrogate Modelling}
\subsection{Convolutional Neural Networks}\label{subsec:LR CNN}
...
However by generating the parameters of the motor it can be rest assured of more precise results. Hence the need to focus on the inputs as they are with their 
...
Similar to my usecase for modelling \ac{EM} of cars, the writers of Ref. \cite{ETA-V-2020} experiments to compute the efficiency map of Toyota Prius have 
...
I do surrogate modelling of the \ac{FEA} simulations which serves as the baseline to eliminate the role of evolutionary algorithms as well for generating designs as 
...
its performance attributes. This would be a useful read when there is a plan to extend the current work with its inverse formulation. 
...
\begin{enumerate}[nosep]
    ...
    prominenton visualizing the results of the Torque curves and compare the \ac{RMSE} values in Sections \ref{sec:Results with Baseline}, \ref{sec:Results with MLP}  among others.
\end{enumerate}
...
\subsection{Evaluation Metrics for Torque KPI}\label{sec:Evaluation Metrics for 2D KPI}
...
\subsection{Evaluation Metrics for Efficiency KPI}\label{sec:Evaluation Metrics for 3D KPI}
...
Furthermore as I am predicting a padded matrix to ensure dimensionality sync across different Efficiency \ac{KPI}s, the grid contains values even outside the boundary of the Efficiency \ac{KPI}.
...
\section{Experiments with MLP}\label{sec:Experiments with MLP}
...
I also would like to point out that the 2 outputs typically would thrice on different learning rates and might be a better approach to have separate learning rates in addition 
...
regularization for the Torque \ac{KPI} whose performance I separately discuss in Section \ref{sec:Ablation Studies}.
...
\section{Results with MLP}\label{sec:Results with MLP}
\subsection{Torque KPI Results with MLP}\label{sec:2D Torque Curve Results with MLP}
...
\subsection{Efficiency KPI Results with MLP}\label{sec:3D Efficiency Grid Results with MLP}
...
\subsection{Torque KPI Results with Baseline}\label{sec:3D Efficiency Grid Results with Baseline}
...
\subsection{Efficiency KPI Results with Baseline}\label{sec:3D Efficiency Grid Results with Baseline}
...
\subsection{Torque KPI Results with Smoothening Loss Regularization}\label{subsec:2D Torque Results with Smoothening Loss Regularization}
...
\subsection{Torque KPI Results with No Loss Regularization}\label{subsec:2D Torque Results with No Loss Regularization}
...
\subsection{Efficiency KPI Results with No Loss Regularization}\label{subsec:3D Efficiency Results with No Loss Regularization}
...
Therefore I presume modelling the usecase as a graph will be more reasonable so that I can exploit graph dynamics and aggregate features that are semantically similar.
...
\begin{figure}[H]
    ...
    \caption{Message Passing in \ac{GNN}s (Source : \cite{PINO-2022})} 
...
\begin{enumerate}
    ...
    \item Graph Generation - Generates synthetic graphs based on initial graph representation. An application I can relate to is the inverse formulation of our problem modelled as a graph.
\end{enumerate}
...
I organize the structure of the case study as follows:
First, I provide an introduction on \ac{GNN}s in general in Introduction.
...
I review the works of Heterogeneous \ac{GNN} in this section.
...
most important factor in this domain for example past medical history. However I note that although this paper uses meta relations it does not factor in edge features.\\
...
I find the Heterogeneous graph to be most apt for our use case with its different node and edge types as it preserves both the structural and semantics of our data. 
This property is crucial in modelling our use case as I will then have similar node types-edge types per topology. 
...
$\mathcal{H}$ be the 2 \ac{KPI}s I need to learn from $\mathcal{D}$. We aim to train a multi-task \ac{GNN} model $\mathcal{M}$ such that $\mathcal{M}$ can deliver 
...
Furthermore, I presume graph representation of the data will be more logical than tabular representation due to its ability to grasp the connections within the \ac{EM} structural properties better.
...
However, I realize that our problem cannot be solved using Homogeneous \ac{GNN} which is relatively simpler and is built on a single node and edge type.
In order to model our problem as a graph, I need to represent it as a Heterogeneous graph. 
...
The challenge is to choose whether I want to represent each component of the \ac{EM} as dedicated nodes and edges to preserve the semantics of the data and be moren flexible 
...
Although the topologies can be represented as different graphs with unique number of nodes and edges for the common node and edge types
...
I have developed a topology invariant \ac{MLP} model to predict 2 \ac{KPI}'s of an \ac{EM} from its parameteric requirements. 
...
It also lays the foundation for future work on being able to generate electric motor design parameters conditioned on the 2 KPIs I had predicted.
I envision that the study will provide new perspectives to researchers engaging in the field of electric motor design and hope my contribution will aid in the same.\\
...
However, it would be computationally expensive and does not help in the scenario when I might need to generate \ac{EM} parametric descriptions.
...
been dependent on any other performance characteristic of the \ac{EM} let alone its parametric description.
