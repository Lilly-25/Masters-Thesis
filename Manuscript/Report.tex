...
Despite the computational burden it comes with, the authors from \cite{FEA-ETA-2017} defends \ac{FEA} to be most appropriate to 
...
\begin{enumerate}
    \item Is it feasible to predict the targets as a multi regression problem with a single model?
    ...
    \item Is it possible to model an array of integer values to be predicted as a regression problem?
    ...
    \item How to handle varying dimensions within a target for model training?
    ...
    \item How to accommodate predictions of targets of varying ranges with a single model?
    ...
    \item With a single model, how to predict 2 targets where the dimensionality of a target is dependent on the other?
    ...
    There has been literature on how the values of a target are affected by the the values of another target. However, our situation is tricky in the sense that the dimensionality 
    of the Efficiency grid is dependent on the values of the Torque curve.
\end{enumerate}
...
approach as can be inferred from \cite{DFIG-2023} where the author presents numerous scenarios of generated images not being upto mark. These scenarios include difficulty to render 
...
In \cite{BFIG-2023}, the authors have attempted to generate images of bicycle frame generation using a Deep Conditional 
...
parametric description. Vivek \textit{et al.} in \cite{VAE-MT-2021} also highlights that predicting \ac{KPI}s with tabular data is significantly more efficient than 
...
The authors of \cite{EM 2DFMP-2022} presents a method to predict 2\ac{D} flux maps of an Interior \ac{PMSM} motor using classical machine learning ensemble regression models.
...
The researchers of \cite{SM EMT-2020} state that the use of domain knowledge improved the accuracy of surrogate modelling \ac{EM} by creating a hybrid of both 
...
Similarly, Yusuke \textit{et al.} in \cite{PANN-MT-2021} and \cite{PANN-MOO-2021} also claim that a physics assisted neural network significantly improved the accuracy 
...
Works by Arbaaz \textit{et al.} in \cite{EM TL-2020} explores methodologies to extend an already model to predict efficiency maps for a topology it was untrained for 
...
Marius \textit{et al.} in \cite{VAE-MT-2021} undertake works on optimizing \ac{EM} design generation by compressing its parametric description 
...
Bucher \textit{et al.} in \cite{GDEM-2023} proposed a deep conditional generative design workflow using a to generate complete parametric 
...
The authors of \cite{VAE-MT-2021} also disclose the solutions taken to address the problem wherein unique parameters of 1 topology are absent in another topology.
...
The writers of \cite{ETA-LA-2020} emphasize the importance of having a very low error rate for the Efficiency Maps by \ac{FEA} simulations as the drive range of a 
...
\begin{enumerate}
    \item The Standard deviation is at its peak at low angular velocities.
    ...
    \item Some Samples Standard Deviation seems to follow the mean curve.
    ...
    prominent when we visualize the results of the Troque curves and compare the \ac{RMSE} values in Sections \ref{sec:Results with Baseline}, \ref{sec:Results with MLP}  among others.
\end{enumerate}
...
\begin{equation}
    \text{$\mathcal{Y}_1$ Percentage Difference} = (\text{$\mathcal{Y}_1$ Score} / {(\mathcal{Y}_1 Max - \mathcal{Y}_1 Min)})  \cdot 100
    \label{eq:Y1 Percentage Difference}
\end{equation}
\begin{equation}
    \text{$\mathcal{Y}_2$ Percentage Difference} = (\text{$\mathcal{Y}_2$ Score} / {(\mathcal{Y}_2 Max - \mathcal{Y}_2 Min)})  \cdot 100
...
Fig. \ref{fig:MLP Training Results for 2D KPI(Torque)} illustrates 6 test dataset predicted Torque values and their corresponding ground truths displayed against their angular velocities.
...
\begin{figure}[H]
    ...
    \caption{Message Passing in \ac{GNN}s (\cite{PINO-2022})} 
...
and interdependency between objects \cite{GNN-2019}.
...
Heterogeneous \ac{GNN}s also takes into account homophily wherein nodes close on a network have similar embeddings \cite{HGNN-2020} this is in essence the structural 
...
When \( |T_v| = |T_e| = 1 \), which implies there is only 1 node and edge type then the graph degenerates into a homogeneous graph \cite{SE HGNN-2023}.
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.35\textwidth}
        ...
        \caption{Metapath} % Center the caption here
        \label{fig:Metapath}
    \end{subfigure}
    \begin{subfigure}{0.35\textwidth}
        \centering
...
Guan \textit{et al.} in \cite{HGNNSG-2023} presents a methodology to learn a heterogeneous graph by decomposing it into homogeneous and heterogeneous subgraphs from its metapaths 
...
Such meta relations are shallow embeddings and not as deep as the meta paths as was clarified by Yang \textit{et al.} in \cite{HGNN-2020}. \\
...
Li \textit{et al.} in \cite{GCN-2018} demystifies the \ac{GCN} and addresses the Laplacian oversmoothing concern by self-training or co-training the model.
...
\cite{GCN-2018} sheds light on the oversmoothing concern faced by \ac{GCN}.
...
Ahn \textit{et al.} in \cite{RHGNN-2022} propose to mitigate oversmoothing by by introducing skip connections between layers in the network to favor node specific embeddings.
Similarly in \cite{REF HGNN-2021}, Lv \textit{et al.} study existing heterogeneous \ac{GNN}s and also cites preactivated residual connections can help resist over-smoothing from happening.
...
Yu \textit{et al.} in \cite{SHGNN-2020} addresses the concern of featureless nodes by padding 0s, imputing the mean of neighboring nodes and using pretrained graph 
embeddings as initial features. 

\subsection{Metapaths}\label{subsec:HGNN Metapaths}
Xu \textit{et al.} in \cite{EMPHGNN-2023} demonstrated a technique of splitting metapaths from inside out to be able to capture the semantic meaning as much as 
...
Yang \textit{et al.} in \cite{SE HGNN-2023} conducted a study on building an efficient Heterogeneous \ac{GNN} by using single layer longer metapaths rather than 
...
Gao \textit{et al.} in \cite{HGNNAS-2021} proposes an architercture search for heterogeneous \ac{GNN}s to effectively choose best network architecture by using 
reinforcement learning. 
The authors of \cite{HGNNRM-2023} propose a relevance measure to capture the sematics of nodes of different types in a heterogeneous graph based on context path 
...
Given the \ac{EM} data \( D\), our goal is to construct a heterogeneous graph \( G\) from \( D\). Let \( T1\), \( T2\) on \( G\) be the 2 \ac{KPI}s we need to 
learn from \( D\). We aim to train a multi-task \ac{GNN} model M such that M can deliver high performance on \( T1\) and \( T2\).
